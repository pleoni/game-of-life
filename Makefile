...
LOADGCC5     = module load gcc51 openmpi-x86_64
UNLOADGCC5   = module unload gcc51 openmpi-x86_64
LOADINTEL   = module load intel intelmpi
UNLOADINTEL = module unload intel intelmpi
UNLOADALL   = $(UNLOADPGI); $(UNLOADGNU); $(UNLOADINTEL); $(UNLOADGCC5)
...
acc_pgi_kep:
	bash -c "$(UNLOADALL) ; $(UNLOADGCC5) ; module load pgi ; \
...
omp4_gcc_kep:
	bash -c "$(UNLOADALL) ; $(LOADGCC5) ; \
	$(CC) $(PACKAGE).c -o $(PACKAGE)_omp4kep $(MyO) -fopenmp -ftree-vectorize -ftree-vectorizer-verbose=1 -D OMP4"

acc_gcc_kep:
	bash -c "$(UNLOADALL) ; $(LOADGCC5) ; \
	$(CC) $(PACKAGE).c -o $(PACKAGE)_accgcckep $(MyO) -fopenacc -ftree-vectorize -ftree-vectorizer-verbose=1"

clean: ; $(RM) $(PACKAGE)_accpgi  $(PACKAGE)_omppgi $(PACKAGE)_ompicc $(PACKAGE)_omp4icc $(PACKAGE)_serpgi $(PACKAGE)_ompgnu $(PACKAGE)_sergnu $(PACKAGE)_omp.mic $(PACKAGE)_acckep $(PACKAGE)_ompkep $(PACKAGE)_omp4kep $(PACKAGE)_accgcckep $(TESTFILE) $(TESTFILE).txt
