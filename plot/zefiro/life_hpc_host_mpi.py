...
top.set_title(str(today) + ' life_hpc_host_mpi on Zefiro (1 host) - NCOMP=1000')
top.grid()
top.set_xlabel('Lattice Size (per MPI process)')
top.set_ylabel('time')
#top.set_yscale('log') 
...
bottom.set_title(str(today) + ' life_hpc_host_mpi on Zefiro (1 host) - NCOMP=0')
bottom.grid()
bottom.set_xlabel('Lattice size (per MPI process)')
